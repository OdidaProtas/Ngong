...
export default function TransferDesktopTable() {
  ...
  const { transfers }: any = React.useContext(StateContext);
  const { push } = useHistory();
  const { id }: any = useParams();

  const rows = transfers?.map((product: any) => {
    const { title, id, status, trackQuantity, quantity, productType, vendor } =
      product;
    return createData(
      title,
      trackQuantity,
