...
export default function CustomerTable() {
  ...
  const { customers }: any = React.useContext(StateContext);
  const { push } = useHistory();
  const { id }: any = useParams();

  const rows = customers?.map((product: any) => {
    const { firstName, id, status, trackQuantity, quantity, productType, vendor } =
      product;
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="right">O orders</TableCell>
        <TableCell align="right">Ksh. 0.0</TableCell>
        <TableCell>
          <IconButton
            disabled
